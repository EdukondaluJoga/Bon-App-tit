...
module.exports = {
    addRestaurant: async (req,res) => {
        ...
        if(!title || !time || !imageUrl || !owner || !code ||!logoUrl || !coords
            || !coords.latitude || !coords.longitude || !coords.address || !coords.title){
                return res.status(400).json({stauts:false, message: "You have some missing field"});
            }
        ...
        try {
            ...
            res.status(201).json({status: true, message: "Restaurant has been added successfully"});
        } catch (error) {
    ...
    getNearByRandomRestaurants: async (req, res) => {
        const code = req.params.code;
        try {
